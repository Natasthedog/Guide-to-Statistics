...
def load_data1():
    data_file1 = st.file_uploader("Upload CSV file 1", type = ["csv"])
    return data_file1

def load_data2():
    data_file2 = st.file_uploader("Upload CSV file 2", type = ["csv"])
    return data_file2
...
def main():
    stc.html(html_temp)
    menu = ["Home", "Exploratory Data Analysis", "Distribution Fitting", "Machine Learning", "About"]
    ...
    if choice == "Home":
        ...
        if data_file is not None:
            ...
            submenu = st.sidebar.radio("Submenu",
                                           ["Descriptive Stats", "Visualization"])

            if submenu == "Descriptive Stats":
                st.header("Descriptive Stats")
                st.write(f"Here is an overview of the first 5 rows of the data set loaded:")
        ...
        if data_file is not None:
            ...
            ohe_sp = pd.get_dummies(df['Processor'])
            ohe_day = pd.get_dummies(df['Day'])
            ohe_month = pd.get_dummies(df['Month'])

            x1 = pd.concat([df['Success Rate'], df['Dollar Sales'], ohe_sp, ohe_day, ohe_month], axis=1)
            X = st.multiselect("Select dependent variables", x1.columns.to_list())
            y = st.selectbox("Select the independent variable")
            X
